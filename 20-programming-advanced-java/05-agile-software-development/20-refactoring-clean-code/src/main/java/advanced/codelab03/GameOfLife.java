...
public class GameOfLife {
			...
			for (int i = 0; i < dish[row].length(); i++) {//each char in the row
				...
				if (above != null) {//no one above
					for (char x : above.toCharArray()) {//each neighbor from above
                        if (x == 'X') {
                            neighbors++;//count it if someone is here
                        }
					}
				}
				...
				newGen[row] += determineNextGenInCell(neighbors, dish[row].charAt(i));
	...
	private static String determineNextGenInCell(int amountOfNeighbors, char currentValue) {
		if (amountOfNeighbors < 2 || amountOfNeighbors > 3) {
			return "-";//<2 or >3 neighbors -> die
		} else if (amountOfNeighbors == 3) {
			return "X";//3 neighbors -> spawn/live
		} else {
