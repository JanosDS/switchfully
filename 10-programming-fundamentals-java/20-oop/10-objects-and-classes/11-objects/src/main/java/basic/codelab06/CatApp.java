...
public class CatApp {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < cats.length; i++) {
            cats[i] = new Cat(String.valueOf(i));
        }
    ...
    public static void killOddCats(Cat[] cats) {
        for (int i = 0; i < cats.length; i++) {
            if (i % 2 == 0) {
                cats[i].kill();
            }
    ...
    public static void killCat(Cat cat) {
        while (cat.hasLivesLeft()) {
            cat.kill();
        }
        System.out.println("Cat " + cat.getName() + " has died... Luckily it's not a real cat.");
    }
}
