...
As said before, Spring Boot aims to get us up and running as quickly as possible (it "just runs").
It does this by packaging an embedded tomcat in our WAR. That way, we don't need to manually deploy WAR files.
...
but it can also be used as an HTTP server.
In short: it will host and run WAR files.

1. Download Apache Tomcat 9.X
2. Install Apache Tomcat
    - The ui-manager should be installed as well (this should happen by default)
3. Run the service
...
4. If Tomcat's successfully running and installed, go to `http://localhost:8080/manager/html`
    - It's possible you have to configure a user first
